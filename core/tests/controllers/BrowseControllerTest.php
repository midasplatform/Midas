...
class BrowseControllerTest extends ControllerTestCase
{
    ...
    public function testMovecopyAction()
    {
        ...
        $this->assertQueryContentContains(
            '#moveCopyTable tr[type="folder"]',
            'My Files ('.$userDao->getFullname().')'
        );
        ...
        $this->resetAll();
        $this->dispatchUri(
            '/browse/movecopy?destination=1002&copytype=copy&duplicateElement=Duplicate&elements=;1001-',
            $userDao
        );
        $oldFolder = $this->Folder->load(1001);
        $newFolder = $this->Folder->load(1002);
        ...
        $this->resetAll();
        $this->dispatchUri(
            '/browse/movecopy?destination=1002&copytype=reference&shareElement=Share&elements=;1001-',
            $userDao
        );
        $oldFolder = $this->Folder->load(1001);
        $newFolder = $this->Folder->load(1002);
        ...
        $this->assertEquals(count($newFolder->getItems()), $oldCountDst + 1); // assert item added to new folder
        $this->assertEquals(
            count($item->getFolders()),
            $oldCountParents + 1
        ); // assert item now has an additional parent
