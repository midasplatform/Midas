...
class Core_ExportComponentTest extends ControllerTestCase
{
    ...
    public function uploadItems($userDao)
    {
        ...
        $license = 0;
        $uploadCompoenent->createUploadedItem(
            $userDao,
            $user1_public_filename,
            $user1_public_path,
            $user1_public_parent,
            $license,
            '',
        ...
        $uploadCompoenent->createUploadedItem(
            $userDao,
            $user1_private_filename,
            $user1_private_path,
            $user1_private_parent,
            $license,
            '',
            true
        );
    ...
    public function testCreateSymlinks()
    {
        ...
        $user1_public_bitstream_path = $user1_public_lastbitstream->getAssetstore()->getPath(
            ).'/'.$user1_public_lastbitstream->getPath();
        $this->assertTrue(is_link($midas_exporttest_dir.'/'.$itemIds[0].'/public.file'));
        $this->assertEquals(
            realpath($user1_public_bitstream_path),
            realpath(readlink($midas_exporttest_dir.'/'.$itemIds[0].'/public.file'))
        );
        ...
        $user1_private_bitstream_path = $user1_private_lastbitstream->getAssetstore()->getPath(
            ).'/'.$user1_private_lastbitstream->getPath();
        $this->assertTrue(is_link($midas_exporttest_dir.'/'.$itemIds[1].'/private.png'));
        $this->assertEquals(
            realpath($user1_private_bitstream_path),
            realpath(readlink($midas_exporttest_dir.'/'.$itemIds[1].'/private.png'))
        );
    ...
    public function testCopy()
    {
        ...
        $this->assertFileEquals(
            $user1_private_bitstream_path,
            $midas_exporttest_dir.'/'.$itemIds[0].'/private.png'
        );
