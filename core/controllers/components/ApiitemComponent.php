...
class ApiitemComponent extends AppComponent
{
    ...
    public function itemDeletemetadataAll($args)
    {
        ...
        if (array_key_exists('revision', $args) && $args['revision'] === 'all'
            ...
            $revisionDao = $apihelperComponent->getItemRevision(
                $item,
                isset($args['revision']) ? $args['revision'] : null
            );
    ...
    public function itemSearch($args)
    {
        ...
        } elseif (array_key_exists('folderName', $args)) {
    ...
    public function itemGetWrapper($args)
    {
        ...
        $out['revisions'] = array_map(array($this, 'getRevisionId'), $in['revisions']);
        $head_revision = end($in['revisions']);
        $out['bitstreams'] = array_map(array($this, 'getBitstreamId'), $head_revision['bitstreams']);
    ...
    public function itemListPermissions($args)
    {
        ...
        return $apihelperComponent->listResourcePermissions(
            ...
            $item->getItempolicyuser(),
            $item->getItempolicygroup()
        );
    }
    ...
    public function itemMove($args)
    {
        ...
        if ($item === false || !$itemModel->policyCheck(
                $item,
                $userDao,
                MIDAS_POLICY_ADMIN
