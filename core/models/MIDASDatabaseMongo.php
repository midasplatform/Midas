...
class MIDASDatabaseMongo implements MIDASDatabaseInterface
{
  ...
  public function initialize($name, $key, $data)
    ...
    if(!isset($this->_mainData))
      {
      throw new Zend_Exception("Model Mongo " . $this->_name . " is not defined properly.");
      }
   ...
   */
  public function fetchRow($queryarray)
    {
    $collection =  $this->_name;
    $db = $this->_db;
    return $db->$collection->findOne($queryarray);
    }
  ...
  public function getValues($key)
    ...
    catch(Mongo_NotFoundException $e)
      {
      return null;
      }
    catch(Exception $e)
  ...
  public function save($dataarray)
    ...
    try
      {
      // There is no update in Mongo, everything is insert by key
  ...
  public function delete($dao)
    ...
    if(!isset($this->_key) || !$this->_key)
      {
      throw new Zend_Exception("MIDASDatabaseMongo::delete() : Cannot delete record by something other than a key." );
      return false;
      }
  ...
  public function getValue($var, $key, $dao)
    ...
    if(!isset($this->_mainData[$var]))
      {
      throw new Zend_Exception("Database Mongo " . $this->_name . ": var ".$var." is not defined here.");
      }
    ...
    if(method_exists($this, 'get' . ucfirst($var)))
      ...
      try
        ...
        if(!isset($resultarray[$var]))
          {
          //echo 'MIDASDatabaseMongo::getValue() MIDAS_DATA not found. CF='.$this->_name.' and va .= '.$var;
          //throw new Zend_Exception('MIDASDatabaseMongo::getValue() MIDAS_DATA not found. CF='.$this->_name.' and va .= '.$var);
          return null;
          }
      ...
      {
      throw new Zend_Exception('MIDASDatabaseMongo: getValue() Unable to load data type ' . $var);
      }
    }

  /** Helper function for Mongo */
  function getMongo($columnfamily, $key, $columns = null, $column_start = "", $column_finish = "")
    {
    try
      {
  ...
  /** multiget */
  function multigetMongo($columnfamily, $keys, $columns = null, $column_start = "", $column_finish = "")
    {
    try
      {
      $cf = new ColumnFamily($this->_db, $columnfamily);
