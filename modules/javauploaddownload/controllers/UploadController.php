...
class Javauploaddownload_UploadController extends Javauploaddownload_AppController
  ...
  public function indexAction()
    ...
    $this->view->extraHtml = Zend_Registry::get('notifier')->callback(
      'CALLBACK_CORE_GET_JAVAUPLOAD_EXTRA_HTML', array('folder' => $folder));
    session_write_close();
    }
  ...
  public function revisionAction()
    ...
    $this->view->extraHtml = Zend_Registry::get('notifier')->callback(
      'CALLBACK_CORE_GET_JAVAREVISIONUPLOAD_EXTRA_HTML', array('item' => $item));
    }
  ...
  function processjavauploadAction()
    ...
    $validations = Zend_Registry::get('notifier')->callback(
      'CALLBACK_CORE_VALIDATE_UPLOAD',  array('filename' => $data['filename'],
                                              'size' => $data['size'],
                                              'path' => $data['path'],
                                              'folderId' => $parentId));
    ...
    if(!empty($data['path']) && file_exists($data['path']) && $data['size'] > 0)
      ...
      try
        ...
        $this->Component->Upload->createNewRevision(
          $this->userSession->Dao, $data['filename'], $data['path'], $changes,
          $parentId, $revNumber, $license, $data['md5'], (bool)$testingMode);
        }
      catch(Exception $e)
        {
        if(!$testingMode)
