...
$('a#setElementPublicLink').click(function() {
  $.post(json.global.webroot+'/remoteprocessing/dashboard/sharedomain', {setPublic:true,type: jsonShare.type, domainId: jsonShare.domain}, function(data) {
...
$('a.removeShareLink').click(function() {
  ...
  $.post(json.global.webroot+'/remoteprocessing/dashboard/sharedomain', {removePolicy:true,removeType:removeType,removeId:removeId, domainId: jsonShare.domain}, function(data) {
    jsonResponse = jQuery.parseJSON(data);
    if(jsonResponse[0])
      {
      createNotice(jsonResponse[1], 1500);
...
$('select.changePermissionSelect').change(function() {
  ...
  var obj = $(this).parents('tr');
  $.post(json.global.webroot+'/remoteprocessing/dashboard/sharedomain', {changePolicy:true,changeVal:changeVal,changeType:changeType,changeId:changeId, domainId: jsonShare.domain}, function(data) {
    jsonResponse = jQuery.parseJSON(data);
    if(jsonResponse[0])
      {
      createNotice(jsonResponse[1], 1500);
...
$('a#setElementPrivateLink').click(function(){
  $.post(json.global.webroot+'/remoteprocessing/dashboard/sharedomain', {setPrivate:true, domainId: jsonShare.domain}, function(data) {
    jsonResponse = jQuery.parseJSON(data);
    if(jsonResponse[0])
      {
      createNotice(jsonResponse[1], 1500);
...
source: function( request, response ) {
 ...
 select: function(event, ui) {
   ...
   $.post(json.global.webroot+'/remoteprocessing/dashboard/sharedomain', {createPolicy:true,newPolicyType:newPolicyType,newPolicyId:newPolicyId, domainId: jsonShare.domain},
     function(data) {
       jsonResponse = jQuery.parseJSON(data);
        if(jsonResponse[0])
          {
