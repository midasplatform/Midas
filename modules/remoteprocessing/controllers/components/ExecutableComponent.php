...
class Remoteprocessing_ExecutableComponent extends AppComponent
{
    ...
    public function initAndSchedule(
        $userDao,
        $executableItemDao,
        $jobName,
        $cmdOptions,
    ...
    public function processScheduledJobParameters($params)
    {
        ...
        $parameters = $jobComponent->initJobParameters(
            'CALLBACK_REMOTEPROCESSING_EXECUTABLE_RESULTS',
            $inputArray,
            $ouputArray,
            $params['params']
        );
    ...
    private function _createScript($parametersList, $commandMatrix, $executable, $ouputArray)
    {
        ...
        foreach ($commandMatrix as $key => $commandList) {
            $command = $executable->getName().' '.implode('', $commandList);
            $command = str_replace(
                '{{key}}',
                '.'.$this->_generateSuffixOutputName($commandList, $parametersList).'.'.$key,
                $command
            );
        ...
        foreach ($tmpOutputArray as $ouput) {
            ...
            foreach ($commandMatrix as $key => $commandList) {
                $ouputArray[] = str_replace(
                    '.'.$ext,
                    '.'.$this->_generateSuffixOutputName($commandList, $parametersList).'.'.$key.'.'.$ext,
                    $ouput
                );
            }
        }
    ...
    /** create Xml File */
    public function createDefinitionFile($elements)
    {
