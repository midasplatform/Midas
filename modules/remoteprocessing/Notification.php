...
class Remoteprocessing_Notification extends ApiEnabled_Notification
{
    ...
    {
        return '<link type="text/css" rel="stylesheet" href="'.Zend_Registry::get(
            'webroot'
        ).'/modules/remoteprocessing/public/css/layout/remoteprocessing.css" />';
    }

    /** get layout footer */
    public function getFooter()
    {
        return '<script type="text/javascript" src="'.Zend_Registry::get(
            'webroot'
        ).'/modules/remoteprocessing/public/js/layout/remoteprocessing.js"></script>';
    }
    ...
    public function getButton($params)
    {
        if ($this->Setting->getValueByName(MIDAS_REMOTEPROCESSING_SHOW_BUTTON_KEY, $this->moduleName) == 1) {
            $html = "<li class='processButton' style='margin-left:5px;' title='Process' rel='".Zend_Registry::get(
                  ...
                  <img id='processButtonLoadiing' style='margin-top:5px;display:none;' src='".Zend_Registry::get(
                    'webroot'
                )."/core/public/images/icons/loading.gif' alt=''/>
                    Process
                  </a>
                </li> ";
    ...
    public function processProcessingResults($params)
    {
                    ...
                    foreach ($params['parametersList'] as $key => $name) {
                        if (isset($params['optionMatrix'][$tmpArray[1]][$key])) {
                            ...
                            $metadataModel->addMetadataValue(
                                ...
                                'parameter',
                                $name,
                                $params['optionMatrix'][$tmpArray[1]][$key]
                            );
                        }
                    }
                }
        ...
        if (isset($params['log']) && !empty($params['log'])) {
            ...
            $item = $uploadComponent->createUploadedItem(
                $userDao,
                'job-'.$params['job_id'].'_results.xml',
                $logFile,
                $folder
            );
        ...
        if (!empty($params['params']['input'])) {
            foreach ($params['params']['input'] as $itemId) {
                ...
                if ($item != false && $item->getKey() != $params['params']['executable']
                    ...
                    $this->Remoteprocessing_Job->addItemRelation(
                        $job,
                        $item,
                        MIDAS_REMOTEPROCESSING_RELATION_TYPE_INPUT
                    );
                } elseif ($item != false) {
                    $this->Remoteprocessing_Job->addItemRelation(
                        $job,
                        $item,
                        MIDAS_REMOTEPROCESSING_RELATION_TYPE_EXECUTABLE
                    );
                }
            }
        }
