...
midas.sizequota.validateUpload = function (args) {
    ...
    if (freeSpace != '' && midas.sizequota.totalSize > parseInt(freeSpace)) {
        if (args.revision) {
            return {
                status: false,
                message: 'Uploading these files would exceed the maximum quota for the parent folder. ' + 'Please remove some of the files.'
            };
        }
        else {
            return {
...
midas.sizequota.folderChanged = function (args) {
    ...
    $.ajax({
        type: 'POST',
        url: json.global.webroot + '/sizequota/folder/getfreespace',
        data: {
