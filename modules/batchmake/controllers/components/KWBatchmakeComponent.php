...
{
    protected static $configPropertiesRequirements = array(
        MIDAS_BATCHMAKE_TMP_DIR_PROPERTY => MIDAS_BATCHMAKE_CHECK_IF_CHMODABLE_RW,
    ...
    protected static $applicationsPaths = array(
        MIDAS_BATCHMAKE_CONDOR_STATUS => MIDAS_BATCHMAKE_CONDOR_BIN_DIR_PROPERTY,
    ...
    public function testconfig($alternateConfigValues = null)
    {
        ...
        $phpProcessUserString = $phpProcessString.' '.InternationalizationComponent::translate(
                MIDAS_BATCHMAKE_PHP_PROCESS_USER_STRING
            );
        ...
        foreach ($processProperties as $property => $value) {
            $status = !empty($value);
            $configStatus[] = array(
                MIDAS_BATCHMAKE_PROPERTY_KEY => $property,
        ...
        foreach (self::$configPropertiesRequirements as $configProperty => $configPropertyRequirement) {
            ...
            if ($configPropertyVal) {
                ...
                list($result, $status) = $this->checkFileFlag(
                    $configPropertyVal,
                    $processUserUid,
                    $configPropertyRequirement
                );
                $configStatus[] = array(
                    MIDAS_BATCHMAKE_PROPERTY_KEY => $configProperty,
                    MIDAS_BATCHMAKE_STATUS_KEY => $status,
                    MIDAS_BATCHMAKE_TYPE_KEY => $result ? MIDAS_BATCHMAKE_STATUS_TYPE_INFO : MIDAS_BATCHMAKE_STATUS_TYPE_ERROR,
                );
                ...
                $configStatus[] = array(
                    MIDAS_BATCHMAKE_PROPERTY_KEY => $configProperty,
                    MIDAS_BATCHMAKE_STATUS_KEY => MIDAS_BATCHMAKE_CONFIG_VALUE_MISSING,
                    MIDAS_BATCHMAKE_TYPE_KEY => MIDAS_BATCHMAKE_STATUS_TYPE_ERROR,
                );
                $total_config_correct = 0;
            }
        }
        ...
        foreach (self::$applicationsPaths as $app => $pathProperty) {
            ...
            $configStatus[] = array(
                MIDAS_BATCHMAKE_PROPERTY_KEY => $applicationString.' '.$appPath,
                MIDAS_BATCHMAKE_STATUS_KEY => $status,
                MIDAS_BATCHMAKE_TYPE_KEY => $result ? MIDAS_BATCHMAKE_STATUS_TYPE_INFO : MIDAS_BATCHMAKE_STATUS_TYPE_ERROR,
            );
            // the property is in error, therefore so is the global config
    ...
    {
        /** @var Batchmake_TaskModel $batchmakeTaskModel */
    ...
    /**
     * will check that the passed in $batchmakescript
    ...
    {
        // Prepare command
        $params = array('-ap', $this->configAppDir, '-p', $workDir, '-c', $workDir.$bmScript);
    ...
    public function condorSubmitDag($workDir, $dagScript)
    {
        ...
        $cmd = KWUtils::prepareExecCommand(
            $this->configCondorBinDir.'/'.MIDAS_BATCHMAKE_CONDOR_SUBMIT_DAG,
            $params
        );
