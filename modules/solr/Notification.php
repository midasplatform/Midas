...
/** Notification manager for the solr module */
class Solr_Notification extends ApiEnabled_Notification
{
    ...
    public function itemSearch($args)
    {
        ...
        $solrQuery = 'name: '.$query.' OR description: '.$query.' OR metadata: '.$query;
    ...
    public function itemDeleted($args)
    {
        ...
        try {
            ...
            $this->getLogger()->warn(
                'Error deleting item refs ('.$item->getKey().') from Solr index: '.$e->getMessage()
            );
        }
    }
    ...
    public function indexFolder($args)
    {
        ...
        try {
            ...
            $this->getLogger()->warn(
                'Error saving folder ('.$folder->getKey().') to Solr index: '.$e->getMessage()
            );
        }
    }
    ...
    public function folderDeleted($args)
    {
        ...
        try {
            ...
            $this->getLogger()->warn(
                $e->getMessage.' - Could not delete folder from index ('.$folder->getKey().')'
            );
        ...
        try {
            ...
            $this->getLogger()->warn(
                'Error deleting folder refs ('.$folder->getKey().') from Solr index: '.$e->getMessage()
            );
        }
    }
