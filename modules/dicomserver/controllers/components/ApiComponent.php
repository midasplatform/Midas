...
class Dicomserver_ApiComponent extends AppComponent
{
  ...
  function start($args)
  {
    ...
    if($running_status['status'] > MIDAS_DICOM_SERVER_NOT_RUNNING &&
  ...
  function status($args)
    ...
    if(!$userDao || !$userDao->isAdmin())
      {
      throw new Exception('Only administrator can check the running status of DICOM server!', MIDAS_INVALID_POLICY);
      }
    $storescp_cmd = 'storescp';
    if(!empty($args['storescp_cmd']))
      {
    ...
    foreach ($output as $line)
      ...
      else if(!strcmp($process, $dcmqrscp_cmd))
        {
