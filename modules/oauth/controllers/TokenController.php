...
class Oauth_TokenController extends Oauth_AppController
{
    ...
    private function _authorizationCode($secret)
    {
        ...
        if (!isset($clientId)) {
            $this->_doOutput(
                array('error' => 'invalid_request', 'error_description' => 'Must pass a client_id parameter')
            );
        ...
        if (!$clientDao) {
            $this->_doOutput(array('error' => 'invalid_client', 'error_description' => 'Invalid client_id'));
        ...
        if (!$codeDao) {
            $this->_doOutput(array('error' => 'invalid_grant', 'error_description' => 'Invalid authorization code'));

            return;
        }
        if ($codeDao->isExpired()) {
            $this->_doOutput(
                array('error' => 'invalid_grant', 'error_description' => 'Authorization code has expired')
            );
        ...
        if ($codeDao->getClientId() != $clientDao->getKey()) {
            $this->_doOutput(
                array('error' => 'invalid_grant', 'error_description' => 'Code does not correspond to this client')
            );
        ...
        $this->getLogger()->debug(
            'New tokens issued to client '.$clientId.' for user '.$accessToken->getUserId()
        );
    }
    ...
    private function _refreshToken($secret)
    {
        ...
        if (!$refreshToken || $refreshToken->getType() != MIDAS_OAUTH_TOKEN_TYPE_REFRESH
            ...
            $this->_doOutput(array('error' => 'invalid_grant', 'error_description' => 'Invalid refresh token'));
        ...
        $this->_doOutput($obj);
        $this->getLogger()->debug(
            'Access token refreshed to client '.$accessToken->getClientId().' for user '.$accessToken->getUserId()
        );
    }
    ...
    private function _doOutput($array)
    {
        if (!headers_sent()) {
            ...
            if (array_key_exists('error', $array)) {
                ...
                $this->getLogger()->crit(
                    'Access token denied ('.$array['error_description'].') '.print_r($this->getAllParams(), true)
                );
            } else {
