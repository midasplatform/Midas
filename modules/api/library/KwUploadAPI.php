...
class KwUploadAPI
{
    ...
    public function setTempDirectory($tmp_directory)
    {
    ...
    public function process($args)
    {
        ...
        if (!array_key_exists(self::PARAM_NAME_UPLOAD_TOKEN, $args)) {
            error_log(
                __FILE__.":".__FUNCTION__.":".__LINE__." - "."Parameter ".self::PARAM_NAME_UPLOAD_TOKEN." is not defined",
                $this->log_type,
                $this->log_file
            );
            throw new Exception(
        ...
        if (!array_key_exists('length', $args)) {
            error_log(
                __FILE__.":".__FUNCTION__.":".__LINE__." - "."Parameter length is not defined",
                $this->log_type,
                $this->log_file
            );
        ...
        if (!file_exists($pathTemporaryFilename)) {
            error_log(
                __FILE__.':'.__FUNCTION__.':'.__LINE__.' - '.'Invalid upload token',
                $this->log_type,
                $this->log_file
            );
        ...
        if ($in === false) {
            error_log(
                __FILE__.':'.__FUNCTION__.':'.__LINE__.' - '."Failed to open source:$inputfile",
                $this->log_type,
                $this->log_file
            );
        ...
        if ($out === false) {
            error_log(
                __FILE__.':'.__FUNCTION__.':'.__LINE__.' - '."Failed to open output file:$pathTemporaryFilename",
                $this->log_type,
                $this->log_file
            );
        ...
        if ($uploadOffset < $length) {
            error_log(
                __FILE__.':'.__FUNCTION__.':'.__LINE__.' - '."Failed to upload file - {$uploadOffset}/{$length} bytes transferred",
                $this->log_type,
                $this->log_file
            );
            throw new Exception(
