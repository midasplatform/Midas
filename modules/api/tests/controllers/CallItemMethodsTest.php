...
class Api_CallItemMethodsTest extends Api_CallMethodsTestCase
{
    ...
    public function testBitstreamUpload()
    {
        ...
        $this->assertTrue(
            file_exists($this->getTempDirectory().'/'.$token),
        ...
        $this->assertEquals(
            $itemDao->getPrivacyStatus(),
            MIDAS_PRIVACY_PRIVATE,
            'Expected a different privacy_status for generated item'
        );
        // delete the newly created item
        $this->Item->delete($itemDao);
        ...
        $this->assertEquals(
            ...
            $description,
            'Expected a different description for generated item'
        );
        $this->assertEquals(
            $itemDao->getPrivacyStatus(),
            MIDAS_PRIVACY_PRIVATE,
    ...
    public function testCreateitemDeleteitem()
    {
        ...
        $this->assertEquals(
            ...
            'Item description is not set correctly'
        );
    ...
    private function _callSetmultiplemetadata($itemId, $metadata, $count = null, $revision = null, $failureCode = null)
    {
        ...
        if (isset($count)) {
            ...
            $this->params['count'] = count($metadata);
        }
    ...
    public function testItemMetadata()
    {
        ...
        $metadata_with_2 = array(
            array('element' => $multiElement1, 'value' => $multiValue1),
            array('element' => $multiElement2, 'value' => $multiValue2),
        );
        $metadata_mismatched = array(
            array('element' => $multiElement1, 'value' => $multiValue1),
        ...
        foreach ($metadata as $metadatum) {
            ...
            foreach ($metadataArray as $storedMetadatum) {
                if ($storedMetadatum->element === $metadatum['element'] && $storedMetadatum->value === $metadatum['value']
                ) {
                    $found = true;
                }
            }
    ...
    public function testItemAddRemovePolicygroup()
    {
        ...
        $params = array('method' => 'midas.item.add.policygroup', 'token' => $this->_loginAsUser($userDao));
        ...
        // test remove
        $params = array('method' => 'midas.item.remove.policygroup', 'token' => $params['token']);
    ...
    public function testItemAddRemovePolicyuser()
    {
        ...
        $params = array('method' => 'midas.item.add.policyuser', 'token' => $this->_loginAsUser($userDao));
        ...
        // test remove
        $params = array('method' => 'midas.item.remove.policyuser', 'token' => $params['token']);
    ...
    public function testItemListPermissions()
    {
        ...
        $params = array('method' => 'midas.item.list.permissions', 'token' => $this->_loginAsUser($userDao));
        $invalidItemId = -10;
        ...
        $requiredParams = array(
            array(
                'name' => 'item_id',
                'valid' => $adminItem->getItemId(),
                'invalid' => $invalidItemId,
            ),
        );
        ...
        $this->assertFalse(
            $itemModel->policyCheck($adminItem, null, MIDAS_POLICY_READ),
            'anonymous user should not have read access to admin item'
        );
        $this->assertFalse(
            $itemModel->policyCheck($adminItem, null, MIDAS_POLICY_WRITE),
            'anonymous user should not have write access to admin item'
        );
        $this->assertFalse(
            $itemModel->policyCheck($adminItem, null, MIDAS_POLICY_ADMIN),
            'anonymous user should not have admin access to admin item'
        );
        $this->assertTrue(
            $itemModel->policyCheck($adminItem, $memberUser, MIDAS_POLICY_READ),
            'member user should have read access to admin item'
        );
        $this->assertFalse(
            $itemModel->policyCheck($adminItem, $memberUser, MIDAS_POLICY_WRITE),
            'member user should not have write access to admin item'
        );
        $this->assertFalse(
            $itemModel->policyCheck($adminItem, $memberUser, MIDAS_POLICY_ADMIN),
            'member user should not have admin access to admin item'
        );
        $this->assertTrue(
            $itemModel->policyCheck($adminItem, $modUser, MIDAS_POLICY_READ),
            'moderator user should have read access to admin item'
        );
        $this->assertTrue(
            $itemModel->policyCheck($adminItem, $modUser, MIDAS_POLICY_WRITE),
            'moderator user should have write access to admin item'
        );
        $this->assertFalse(
            $itemModel->policyCheck($adminItem, $modUser, MIDAS_POLICY_ADMIN),
            'moderator user should not have admin access to admin item'
        );
        ...
        $this->assertTrue(
            $itemModel->policyCheck($adminItem, $userDao, MIDAS_POLICY_ADMIN),
            'admin user should have admin access to admin item'
        );
