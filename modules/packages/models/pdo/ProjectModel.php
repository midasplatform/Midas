...
class Packages_ProjectModel extends Packages_ProjectModelBase
{
    ...
    {
        $sql = $this->database->select()->setIntegrityCheck(false)->from(array('p' => 'packages_project'))->where(
            'enabled = ?',
            1
        )->joinLeft(array('c' => 'community'), 'p.community_id=c.community_id')->order('c.name', 'ASC');
