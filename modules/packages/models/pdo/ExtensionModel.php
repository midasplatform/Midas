...
class Packages_ExtensionModel extends Packages_ExtensionModelBase
{
    ...
    public function get(
        ...
        $sqlCount = $this->database->select()->from(array($this->_name), array('count' => 'count(*)'));
        foreach (array(
                     'extension_id',
                     'os',
                     'arch',
            ...
            if (array_key_exists($option, $params) && $params[$option] != 'any'
                ...
                if ($option == 'category') { // category searches by prefix and among a list of categories
                    ...
                    $filterClause = "packages_extension.category = '".$category."'"." OR packages_extension.category LIKE '".$category.".%'"." OR packages_extension.category LIKE '".$category.";%'"." OR packages_extension.category LIKE '%;".$category.".%'"." OR packages_extension.category LIKE '%;".$category.";%'"." OR packages_extension.category LIKE '%;".$category."'";
    ...
    {
        $sql = $this->database->select()->from(array('e' => 'packages_extension'), array('category'))->where(
            'application_id = ?',
            $applicationId
        )->where('category != ?', '')->distinct();
        $categories = array();
        $rowset = $this->database->fetchAll($sql);
        foreach ($rowset as $row) {
    ...
    {
        $sql = $this->database->select()->from(array('e' => 'packages_extension'), array('release'))->where(
            'application_id = ?',
            $applicationId
        )->where('e.release != ?', '')->distinct();
        $releases = array();
        $rowset = $this->database->fetchAll($sql);
        foreach ($rowset as $row) {
