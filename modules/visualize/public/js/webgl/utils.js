...
// render
var timeRendering = new Date().getTime();
var previousTimeRendering = new Date().getTime();

// progress bar
var progresBarStep;progressBar = 0;
...
function initAxes(divObj)
  ...
  rendererAxes.sortObjects = false;
  rendererAxes.autoClear = false;
  divObj.append( rendererAxes.domElement );

  cameraAxes = new THREE.PerspectiveCamera( 70, 100 / 100, 1, 1000 );
  cameraAxes.position.y = 50;
  cameraAxes.position.z = 200;
  ...
  sceneAxes.add( parent );

  text3d = new THREE.TextGeometry( "X", {
  ...
  text3d = new THREE.TextGeometry( "Z", {
...
// init window and mouse interactions
...
function resetCameraPosition(objects)
  {
      var xmin = null;
      var xmax = null;
      var ymin = null;
      ...
      $.each(objects, function(key, value) {
         ...
         if ( radiusMax == null || radiusMax < value.geometry.boundingSphere.radius  )
           {
           radiusMax = value.geometry.boundingSphere.radius ;
           }
      });

      focalpoint = new THREE.Vector3( xmax - (xmax-xmin)/2 , ymax - (ymax-ymin)/2, zmax - (zmax-zmin)/2 );
      controls.target = focalpoint;
      controls.update();
      camera.position.z = zmax - (zmax-zmin)/2;
      camera.position.x = xmax - (xmax-xmin)/2 ;
      camera.position.y = - radiusMax * 2.5 + ymax - (ymax-ymin)/2;
...
function zoomOut()
  {
    ...
    eye.multiplyScalar( 1.3 );
    camera.position.add( controls.target, eye );
  }

function zoomIn()
  {
    var eye = camera.position.subSelf( controls.target );
    eye.multiplyScalar( 0.7 );
    camera.position.add( controls.target, eye );
  }
...
function cameraUp()
  {
    ...
    quaternion.setFromAxisAngle( new THREE.Vector3(1, 0, 0), 0.05 );
    var eye = camera.position.subSelf( controls.target );
...
// event when windows resized. Please define  onWindowResize which sends width and height
function onWindowResizeWebGL( event )
  {
  if(typeof onWindowResize == 'function')
    {
    var sizes = onWindowResize();
    }
  else
    {
    return;
  ...
  if ( focalpoint != undefined && cameraAxes != undefined)
    {
      ...
      cameraAxes.position.x = normalizedDirection.normalize().x * 200;
      cameraAxes.position.y = normalizedDirection.normalize().y * 200;
...
function loaderElements(files)
  ...
  if(files.length != 0)
    ...
    for( i = 0; i < files.length; i++ )
      {
      loader.load( files[i][0], files[i][1] );
      }
    }
  }
...
function incrementProgressBar()
  {
  progressBar = progressBar + progresBarStep;
  $('#progressBarWebgl').show();
  $('#progressBarWebgl').val(progressBar);
  if(progressBar >= 100)
    {
