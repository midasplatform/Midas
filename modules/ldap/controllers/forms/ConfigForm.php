...
class Ldap_ConfigForm extends AppForm
{
    ...
    public function createConfigForm()
    {
        ...
        $form->setAction($this->webroot.'/ldap/config/index')->setMethod('post');

        $hostname = new Zend_Form_Element_Text('hostname');
        $hostname->setRequired(true)->addValidator('NotEmpty', true);

        $port = new Zend_Form_Element_Text('port');
        $port->setRequired(true)->addValidator('NotEmpty', true);

        $basedn = new Zend_Form_Element_Text('basedn');
        $basedn->setRequired(true)->addValidator('NotEmpty', true);

        $protocolVersion = new Zend_Form_Element_Text('protocolVersion');
        $protocolVersion->setRequired(true)->addValidator('NotEmpty', true);

        $search = new Zend_Form_Element_Text('search');
        $search->setRequired(true)->addValidator('NotEmpty', true);
        ...
        $autoAddUnknownUser->addMultiOptions(array('true' => 'true', 'false' => 'false'));

        $useActiveDirectory = new Zend_Form_Element_Select('useActiveDirectory');
        $useActiveDirectory->addMultiOptions(array('true' => 'true', 'false' => 'false'));
