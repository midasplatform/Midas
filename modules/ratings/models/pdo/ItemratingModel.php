...
class Ratings_ItemratingModel extends Ratings_ItemratingModelBase
{
     ...
     */
    public function getAggregateInfo($item)
    {
        $sql = $this->database->select()->from(
            array('i' => 'ratings_item'),
            array('count' => 'count(*)', 'avg' => 'avg(rating)')
        )->where(
            'item_id = ?',
            $item->getKey()
        );
        ...
        $sql = $this->database->select()->from(
            array('i' => 'ratings_item'),
            array('count' => 'count(*)', 'rating' => 'rating')
        )->where(
            'item_id = ?',
            $item->getKey()
        )->group('rating')->order('rating ASC');
    ...
    {
        $sql = $this->database->select()->from(array('i' => 'ratings_item'), array('rating'))->where(
            'item_id = ?',
            $item->getKey()
        )->where(
            'user_id = ?',
